...
const defaultFields: MutualNdaFields = {
  party1: { ...emptyParty },
  party2: { ...emptyParty },
...
function mndaTermDisplay(data: MutualNdaFields): string {
...
function confidentialityTermDisplay(data: MutualNdaFields): string {
...
function confidentialityTermClause(data: MutualNdaFields): string {
  return data.confidentialityTerm === "years"
...
function summarySections(data: MutualNdaFields): FieldSummaryItem[] {
  return [
...
function parties(data: MutualNdaFields): PartyBlock[] {
  return [
    { label: "Party 1", data: data.party1, fieldConfig: DEFAULT_PARTY_FIELD_CONFIG },
    { label: "Party 2", data: data.party2, fieldConfig: DEFAULT_PARTY_FIELD_CONFIG },
  ];
}

// Standard Terms prose, transcribed from templates/Mutual-NDA.md with the
...
// shared renderer never needs to know about template syntax).
function bodySections(data: MutualNdaFields): DocumentSection[] {
  const purpose = data.purpose || "[Purpose not specified]";
  const governingLaw = data.governingLaw || governingLawFallback;
  const jurisdiction = data.jurisdiction || jurisdictionFallback;
  ...
  return [
    ...
    },
    {
      ...
      body: `This MNDA commences on the ${formatDisplayDate(data.effectiveDate, effectiveDateFallback)} and expires at the end of ${mndaTermDisplay(data)}. Either party may terminate this MNDA for any or no reason upon written notice to the other party. The Receiving Party’s obligations relating to Confidential Information will survive ${confidentialityTermClause(data)}, despite any expiration or termination of this MNDA.`,
    },
    {
...
const content: DocumentContent<MutualNdaFields> = {
  documentType: "mutual-nda",
  title: "Mutual Non-Disclosure Agreement",
  pdfFilename: "Mutual-NDA.pdf",
  summaryHeading: "Cover Page",
  defaultFields,
  hydrate: (raw) => ({
    party1: readGroup(raw, "party1", defaultFields.party1),
    party2: readGroup(raw, "party2", defaultFields.party2),
    purpose: asString(raw.purpose, defaultFields.purpose),
    effectiveDate: asString(raw.effectiveDate, defaultFields.effectiveDate),
    mndaTerm: asEnum(raw.mndaTerm, ["expires", "continues"] as const, defaultFields.mndaTerm),
    mndaTermYears: asInt(raw.mndaTermYears, defaultFields.mndaTermYears),
    confidentialityTerm: asEnum(
      raw.confidentialityTerm,
      ["years", "perpetuity"] as const,
      defaultFields.confidentialityTerm,
    ),
    confidentialityTermYears: asInt(raw.confidentialityTermYears, defaultFields.confidentialityTermYears),
    governingLaw: asString(raw.governingLaw, defaultFields.governingLaw),
    jurisdiction: asString(raw.jurisdiction, defaultFields.jurisdiction),
  }),
  summarySections,
  parties,
  bodySections,
